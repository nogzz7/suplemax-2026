...
// ========== UTILITÁRIO PARA CONVERTER PARA NÚMERO ==========
...
function openProductModal(productId) {
  ...
  document.getElementById('modal-category').textContent = cat ? cat.name : (categoryNames[product.collection] || product.collection);
...
function renderProducts() {
  ...
  container.innerHTML = paginated.map(product => {
    ...
    const salePercentage = product.sale_percentage || 
      Math.round((1 - toNumber(product.price) / toNumber(product.original_price)) * 100);
    const cat = categories.find(c => c.slug === product.collection);
...
function addToCart(productId) {
  ...
  if (existing) {
    ...
    if (existing.quantity >= max) return showNotification(`Limite de ${max} unidades!`, 'error');
    existing.quantity++;
  } else {
    cart.push({ id: product.id, name: product.name, price: product.price, image: product.image_1 || product.image_url, quantity: 1 });
  }
  updateCartUI();
...
function setupEvents() {
  ...
  document.getElementById('search-input').addEventListener('input', e => {
    ...
    const container = document.getElementById('products-grid');
    if (!filtered.length) {
      container.innerHTML = `<div style="grid-column:1/-1; text-align:center; padding:3rem;"><div style="font-size:64px; color:var(--gray-300);"><i class="fas fa-search"></i></div><h3>Nenhum produto encontrado</h3><p>Tente buscar por outro termo</p></div>`;
      document.getElementById('load-more').style.display = 'none';
    } else {
